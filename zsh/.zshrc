...
export PATH="/usr/local/share/git-core/contrib/diff-highlight:$HOME/Documents/git-submodule-update-checker:$HOME/Documents/git-context-switch:$HOME/perl5/bin:$HOME/.ndenv/bin:$HOME/.plenv/bin:$HOME/go/bin:$HOME/.nimble/bin:$PATH"
...
# money
alias money="ruby -e 'C=\`stty size\`.scan(/\d+/)[1].to_i;S=\"\xF0\x9F\x92\xB4\";a={};puts \"\033[2J\";loop{a[rand(C)]=0;a.each{|x,o|;a[x]+=1;print \"\033[#{o};#{x}H \033[#{a[x]};#{x}H#{S} \033[0;0H\"};\$stdout.flush;sleep 0.01}'"
...
if whence abbrev-alias > /dev/null; then
    ...
    abbrev-alias -g -e M='$(git log --graph --pretty=format:"%h %d" HEAD^ | grep "^* \+[0-9a-f]\+ \+(" | head -n 1 | sed "s/^.*(\([^,)]*\)[,)].*$/\1/")'
else
